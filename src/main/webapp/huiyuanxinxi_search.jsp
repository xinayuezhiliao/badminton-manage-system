...
<body>
    ...
    <div class="container">
        ...
        <div class="search-box">
            <form action="<%=basePath%>huiyuanxinxi?action=doSearch" method="post" class="search-form">
                <input type="text" name="huiyuanbianhao" class="search-input" 
                       placeholder="请输入会员编号进行精准查找" 
                       value="<%=searchKey%>" />
                <button type="submit" class="search-btn">🔍 查询</button>
                <a href="<%=basePath%>huiyuanxinxi?action=list" class="back-btn">返回列表</a>
            </form>
        </div>
        ...
        <div class="result-box">
            <%
                List<Huiyuanxinxi> huiyuanList = (List<Huiyuanxinxi>) request.getAttribute("huiyuanxinxiList");
            ...
            %>
                ...
                <div class="result-info">
                    ...
                    📊 共找到 <strong style="color: #16a085;"><%=huiyuanList.size()%></strong> 条记录
                </div>
                <table>
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        <%
                            for (int j = 0; j < huiyuanList.size(); j++) {
                                Huiyuanxinxi u = huiyuanList.get(j);
                                if (u == null) continue;
                        %>
                        <tr>
                            <td><%=(j+1)%></td>
                            ...
                            <td><%=u.getXingbie() != null ? u.getXingbie() : ""%></td>
                            <td><%=u.getHuiyuandengji() != null ? u.getHuiyuandengji() : ""%></td>
                            <td><%=u.getShouji() != null ? u.getShouji() : ""%></td>
                            <td>
                                <% if (u.getId() != null) { %>
            ...
            %>
                <div class="empty-state">
                    <div class="icon">😕</div>
                    <p>未找到符合条件的会员信息</p>
                    <p style="font-size: 15px; color: #7f8c8d;">请检查输入的会员编号是否正确，或尝试其他关键词</p>
                </div>
            <%
                }
            %>
